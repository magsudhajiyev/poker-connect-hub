...
export const useGameStateUI = (initialGameState?: GameState | null): GameStateUIUpdates => {
  ...
  const isPlayerActive = useCallback(
    (position: string): boolean => {
      ...
      const isCurrentPlayer =
        standardizePosition(position) === standardizePosition(currentPlayerPosition);

      // Check if player is still active in the hand
      const player = gameState.activePlayers.find(
        (p) => standardizePosition(p.position) === standardizePosition(position),
      );
  ...
  const initializeGame = useCallback(
    (players: Player[], smallBlind: number, bigBlind: number): GameState => {
