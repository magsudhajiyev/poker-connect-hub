...
import {
  ...
  UnifiedGameState,
  PokerAction,
  LegalActionsResponse as _LegalActionsResponse,
} from '@/types/unified';
...
export const usePokerApiEngine = ({
  ...
  const convertPlayers = useCallback(
    (inputPlayers: LegacyPlayer[] | UnifiedPlayer[]): UnifiedPlayer[] => {
  ...
  const initializeGame = useCallback(async () => {
    ...
    try {
      ...
      const newGameState = pokerApiService.createGameStateFromPlayers(unifiedPlayers, sb, bb);

      // Post blinds
      const sbPlayerIndex = unifiedPlayers.findIndex(
        (p) =>
          p.position.toLowerCase().includes('sb') || p.position.toLowerCase() === 'small blind',
      );
      const bbPlayerIndex = unifiedPlayers.findIndex(
        (p) => p.position.toLowerCase().includes('bb') || p.position.toLowerCase() === 'big blind',
      );
  ...
  const executeAction = useCallback(
    async (actionType: string, amount?: number): Promise<boolean> => {
        ...
        switch (actionType.toLowerCase()) {
          ...
          case 'bet':
            if (amount && amount > 0) {
              ...
              updatedGameState.currentBet = Math.max(
  ...
  const validateAction = useCallback(
    async (playerId: string, action: string, amount?: number): Promise<boolean> => {
      if (!gameStateRef.current) {
        return false;
      }
