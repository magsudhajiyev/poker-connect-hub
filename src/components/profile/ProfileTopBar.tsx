...
export const ProfileTopBar = () => {
  ...
  return <header className="fixed top-0 left-0 right-0 bg-slate-900/80 backdrop-blur-lg border-b border-slate-700/20 z-50">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          {/* Logo and Hamburger Menu */}
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="icon" onClick={toggleSidebar} className="hidden lg:flex text-slate-400 hover:text-slate-200 w-10 h-10 flex-shrink-0">
          ...
          <div className="hidden md:block flex-1 max-w-md mx-4">
            <div className="relative">
              ...
              <Input type="text" value={searchValue} onChange={e => setSearchValue(e.target.value)} className="w-full pl-10 bg-slate-800/40 border-slate-700/30 text-slate-200 placeholder-slate-400 focus:ring-violet-500/50" placeholder="Search hands, players, tags..." />
            </div>
          </div>
          
          {/* Navigation Icons */}
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="sm" onClick={handleHomeClick} className="text-slate-400 hover:text-slate-200 w-10 h-10 flex-shrink-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 bg-slate-800/95 backdrop-blur-sm border-slate-700/50 text-slate-200">
                <DropdownMenuItem onClick={handleProfileClick} className="flex items-center gap-2 cursor-pointer hover:bg-slate-700/50">
                  <User className="w-4 h-4" />
                  Profile
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleSettingsClick} className="flex items-center gap-2 cursor-pointer hover:bg-slate-700/50">
