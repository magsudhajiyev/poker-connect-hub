...
type EventTypeString = 'HAND_INITIALIZED' | 'BLINDS_POSTED' | 'ACTION_TAKEN' | 'STREET_COMPLETED' | 'HAND_COMPLETED';
...
export class EventSourcingAdapter {
  ...
  private async initializeEngine(): Promise<void> {
    ...
    const initEvent = await HandEvent
      .findOne({ 
        handId: handObjectId,
  ...
  async rebuildState(): Promise<EngineState> {
    ...
    const pokerEvents: PokerEvent[] = events.map((event: any) => ({
...
return false;
      ...
      if (action === ActionType.RAISE) {
        const minRaise = state.currentState.betting.minRaise || state.currentState.betting.lastRaiseSize || state.currentState.gameConfig.blinds.big;
        if (amount < minRaise) {
          return false;
        }
      }
