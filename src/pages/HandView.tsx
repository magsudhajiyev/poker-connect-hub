...
const HandViewContent = () => {
  ...
  const dummyFormData: ShareHandFormData = {
  ...
  const dummyTags = ['cash-game', 'button-vs-bb', 'nut-straight', 'value-betting', 'hand-reading'];
  ...
  const calculatePotSize = () => {
    ...
    // Add preflop actions
    dummyFormData.preflopActions.forEach(action => {
    ...
    // Add flop actions
    dummyFormData.flopActions.forEach(action => {
      if (action.betAmount && (action.action === 'bet' || action.action === 'raise' || action.action === 'call')) {
        pot += parseFloat(action.betAmount);
      }
    });
    ...
    // Add turn actions
    dummyFormData.turnActions.forEach(action => {
      if (action.betAmount && (action.action === 'bet' || action.action === 'raise' || action.action === 'call')) {
        pot += parseFloat(action.betAmount);
      }
    });
    ...
    dummyFormData.riverActions.forEach(action => {
