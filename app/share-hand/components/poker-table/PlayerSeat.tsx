...
const PlayerSeat = ({
  ...
  const isShortStack = stackDisplay.amount / displayValues.bigBlind < 10;
    ...
    >
      ...
      <div className={`flex flex-col items-center space-y-1 ${isActive ? 'z-30' : 'z-20'}`}>
        ...
        >
          {/* Player Name */}
          <div
            className={`${isMobile ? 'text-[7px]' : 'text-[9px] sm:text-[10px]'} font-medium text-center ${isMobile ? 'max-w-8' : 'max-w-12'} truncate ${
          ...
          {/* Stack Size */}
          <div
            className={`${isMobile ? 'text-[7px]' : 'text-[9px] sm:text-[10px]'} font-bold ${
              isActive
                ? 'text-emerald-200'
                : player.isHero
                  ? 'text-blue-200'
        ...
        {/* Position Label */}
        <div
          className={`${isMobile ? 'text-[8px]' : 'text-xs'} text-slate-300 font-bold bg-slate-700/50 px-1 py-0.5 rounded`}
        >
        ...
        {isActive && (
          <div
            className={`absolute -top-1 -right-1 ${isMobile ? 'w-3 h-3' : 'w-4 h-4'} bg-emerald-400 rounded-full animate-pulse shadow-lg border-2 border-emerald-200`}
          />
        )}
      </div>
    </div>
