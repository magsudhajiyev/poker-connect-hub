      ...
      {actions.map((actionStep, index) => {
        ...
        return (
          <div
            ...
            className="bg-slate-900/60 rounded-lg p-2 sm:p-3 border border-slate-700/30"
          >
            <div className="flex items-center justify-between mb-1 sm:mb-2">
              <span
                className={`font-medium text-sm sm:text-base ${actionStep.isHero ? 'text-emerald-400' : 'text-violet-400'}`}
              >
                {actionStep.playerName} ({getPositionName(playerPosition || '')})
              </span>
              {actionStep.completed && <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400" />}
            </div>
            ...
            {actionStep.action && (
              <div className="flex items-center space-x-2">
                ...
                {actionStep.betAmount &&
                  (actionStep.action === 'bet' ||
                    actionStep.action === 'raise' ||
                    actionStep.action === 'call') && (
